...
CREATE TABLE IF NOT EXISTS quantnova.pnl_snapshots (
    ...
    CONSTRAINT pnl_snapshots_portfolio_fk FOREIGN KEY (portfolio_id) REFERENCES screenerx.portfolios (id) ON DELETE CASCADE,
    CONSTRAINT pnl_snapshots_broker_fk FOREIGN KEY (broker_account_id) REFERENCES quantnova.broker_accounts (id) ON DELETE SET NULL
);
